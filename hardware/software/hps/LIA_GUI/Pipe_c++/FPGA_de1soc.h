	...
	public:	
		...
		void set_clk_from_frec (int frecuencia)
		{
			...
			for (frec = 1; frec <= 65; frec = frec + 1)
            {
				...
				{
					double error = std::abs(frecuencia - (double)frec * 1000 / (double)(divisor));
					if (error == 0)
					{
						ready_flag=1;
						frec_final = frec;
		...
		// frecuencia debe estar en kHz
		void set_clk_from_frec_and_M (int frecuencia, int M)
		{
			int frec,divisor;
			...
			for (frec = 1; frec <= 65; frec = frec + 1)
            {
				for(divisor = min_divisor; divisor <= max_divisor; divisor++)
				{
					double error = std::abs(frecuencia - (double)frec * 1000 / (double)(M*divisor));
					if (error == 0)
					{
						ready_flag=1;
						frec_final = frec;
